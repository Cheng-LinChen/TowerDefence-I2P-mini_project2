...
extern int freeze_num;//frz

extern int power_num;//frz
...
void ArmySelectScene::Initialize() {
    ...
    std::string s = "Space: " + std::to_string(usedSpace) + "/" + std::to_string(totalSpace);
    AddNewObject(UISpaceUsage = new Engine::Label(s, "pirulen.ttf", 30, 1395, 100, 0, 0, 0, 255, 0.5, 0.5));
    ...
    btn = new Engine::ImageButton("stage-select/dirt.png", "stage-select/floor.png", 1300, 150, 190, 80);
    btn->SetOnClickCallback(std::bind(&ArmySelectScene::PlayOnClick, this, BUTTON_MEDIUM, -1, 0));
    AddNewControlObject(btn);
    AddNewObject(new Engine::Label("MEDIUM", "pirulen.ttf", 30, 1395, 190, 0, 0, 0, 255, 0.5, 0.5));

    btn = new Engine::ImageButton("stage-select/dirt.png", "stage-select/floor.png", 1300, 300, 190, 80);
    btn->SetOnClickCallback(std::bind(&ArmySelectScene::PlayOnClick, this, BUTTON_HARD, -1, 0));
    AddNewControlObject(btn);
    AddNewObject(new Engine::Label("Hard", "pirulen.ttf", 30, 1395, 340, 0, 0, 0, 255, 0.5, 0.5));
    //ex func
...
void ArmySelectScene::PlayOnClick(ButtonType type, int id, int spaceCost) {
    if (type == BUTTON_ENTER) {
        ...
        if (armyAmount[id] > 0 && usedSpace > 0) {
            armyAmount[id]--;
            usedSpace--;
